  ...
  try {
    ...
    elementsToNumber.forEach((el) => {
      ...
      if (context && context.resolvedNumbering && context.abstractNumId) {
        // Check if element is inside a table cell - if so, don't apply bullet formatting
        const isInTableCell = el.closest('td, th, table');
        const format = context.resolvedNumbering.format || "";
        
        // Skip bullet formatting for elements inside table cells
        if (isInTableCell && format === "bullet") {
          console.log(`Skipping bullet formatting for table cell content: "${el.textContent.trim().substring(0, 50)}..."`);
          return; // Don't apply any numbering attributes to table cell content
        }
        ...
        // Add box model fixes to prevent character overlap (but not for bullet lists)
        if (format !== "bullet") {

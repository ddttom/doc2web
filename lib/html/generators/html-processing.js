...
async function applyStylesAndProcessHtml(
  ...
  try {
    ...
    if (options.preserveMetadata && metadata) {
      applyMetadataToHtml(document, metadata, options);
    }
    ...
    // Add return to top button functionality
    addReturnToTopButton(document, styleInfo);
...
function postProcessBulletLists(document, styleInfo) {
  try {
    ...
    // Exclude paragraphs inside table cells
    const bulletParagraphs = Array.from(document.querySelectorAll('p[data-format="bullet"], p.docx-bullet-item, p[class*="docx-bullet-"]'))
      .filter(p => !p.closest('td, th, table'));
    
    if (bulletParagraphs.length === 0) return;
    
    console.log(`Post-processing ${bulletParagraphs.length} bullet paragraphs (excluding table cells)`);
...
function processExistingBulletElements(document, styleInfo) {
  try {
    ...
    // Exclude elements inside table cells
    const bulletElements = Array.from(document.querySelectorAll('li[data-format="bullet"]'))
      .filter(li => !li.closest('td, th, table'));
    
    if (bulletElements.length === 0) return;
    
    console.log(`Processing ${bulletElements.length} existing bullet elements (excluding table cells)`);
...
/**
 * Add return to top button with scroll detection functionality
 * 
 * @param {Document} document - HTML document
 * @param {Object} styleInfo - Style information for theming
 */
function addReturnToTopButton(document, styleInfo) {
  try {
    console.log('Adding return to top button functionality');
    
    // Create the return to top button element
    const button = document.createElement('button');
    button.id = 'return-to-top';
    button.className = 'docx-return-to-top';
    button.setAttribute('aria-label', 'Return to top of document');
    button.setAttribute('title', 'Return to top');
    button.setAttribute('tabindex', '0');
    
    // Create button content
    const icon = document.createElement('span');
    icon.className = 'return-to-top-icon';
    icon.textContent = '↑';
    
    const srText = document.createElement('span');
    srText.className = 'sr-only';
    srText.textContent = 'Return to top';
    ...
    script.textContent = `
      (function() {
          ...
          if (!ticking) {
            requestAnimationFrame(function() {
              ...
              if (shouldShow !== isVisible) {
                ...
                returnToTopButton.classList.toggle('visible', isVisible);
              }
        ...
        // Keyboard support
        returnToTopButton.addEventListener('keydown', function(e) {
        ...
        console.log('Return to top button functionality initialized');
        }
    ...
    console.log('Return to top button added successfully');
    
  } catch (error) {
    console.error('Error adding return to top button:', error);
  }
}

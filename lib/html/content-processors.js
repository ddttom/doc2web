    ...
    headings.forEach((heading) => {
      ...
      if (context && context.resolvedNumbering && context.abstractNumId) {
        ...
        if (context.resolvedNumbering.sectionId) {
          ...
          // Add a wrapper span for the numbering to help with styling
          const wrapper = doc.createElement("span");
          wrapper.className = "heading-number";
          wrapper.setAttribute("aria-hidden", "true");
          
          // Add the numbering content to the wrapper span
          if (context.resolvedNumbering.fullNumbering) {
            wrapper.textContent = context.resolvedNumbering.fullNumbering;
          }
          
          // Move the heading content to a new span
          const contentSpan = doc.createElement("span");
          contentSpan.className = "heading-content";
          contentSpan.innerHTML = heading.innerHTML;
          
          // Clear the heading and add the new structure
          heading.innerHTML = "";
...
function processTOCEntry(entry, index, tocContainer, styleInfo, numberingCtx) {
  try {
    ...
    if (
      ...
      // Add the numbering directly to the text content
    ...
    if (textSpan && !textSpan.hasAttribute("role")) {
      ...
      if (numberingCtx && numberingCtx.resolvedNumbering && numberingCtx.resolvedNumbering.sectionId) {
        ...
        hrefTarget = textSpan.textContent
          .trim()
          .toLowerCase()
          .replace(/\s+/g, "-")

// lib/html/content-processors.js - Enhanced content element processors with proper indentation handling
...
function processHeadings(doc, styleInfo, numberingContext = null) {
  try {
    ...
    Array.from(headings).forEach(heading => {
      ...
      // Apply indentation if the heading has numbering context
      if (context && context.numberingId) {
        heading.setAttribute('data-num-id', context.numberingId);
        heading.setAttribute('data-num-level', context.numberingLevel?.toString() || '0');
      }
...
function processNestedNumberedParagraphs(doc, styleInfo, numberingContext = null) {
  try {
    ...
    // First, process all paragraphs to apply numbering attributes
    processAllParagraphsForNumbering(doc, numberingContext, styleInfo);
    
    // Then, group numbering contexts by numbering ID to create separate lists
    ...
    // Finally, process non-numbered paragraphs that need indentation
...
/**
 * Process all paragraphs to apply numbering attributes
 * @param {Document} document - DOM document
 * @param {Array} numberingContext - Numbering context
 * @param {Object} styleInfo - Style information
 */
  ...
  try {
    ...
    paragraphs.forEach(p => {
      ...
      const context = numberingContext.find(ctx => {
        ...
        return ctxText === pText || 
               ctxText.includes(pText) || 
               pText.includes(ctxText);
      });
      
      if (context && context.numberingId) {
        // Apply numbering attributes
        p.setAttribute('data-num-id', context.numberingId);
        p.setAttribute('data-num-level', context.numberingLevel?.toString() || '0');
        
        if (context.resolvedNumbering) {
          p.setAttribute('data-number', context.resolvedNumbering.rawNumber.toString());
          p.setAttribute('data-format', context.resolvedNumbering.format);
        }
      }
    });
  } catch (error) {
    console.error("Error processing paragraphs for numbering:", error.message);
  }
}

/**
 * Process paragraphs that need indentation but aren't numbered
 * @param {Document} document - DOM document
 * @param {Object} styleInfo - Style information
 * @param {Array} numberingContext - Numbering context
 */
function processIndentedParagraphs(doc, styleInfo, numberingContext) {
  try {
    ...
    paragraphs.forEach(p => {
      // Skip if already processed as list item
      ...
      const numLevel = p.getAttribute('data-num-level');
      
      if (numId && numLevel !== null) {
        // This is a numbered paragraph
        lastNumberedId = numId;
        lastNumberedLevel = parseInt(numLevel, 10);
      } else if (lastNumberedId !== null) {
        // This paragraph follows a numbered item
        // Check if it should inherit indentation
        const pText = p.textContent.trim().toLowerCase();
        
        // Common patterns that indicate continued content
        if (pText.startsWith('whereas') || 
            pText.startsWith('resolved') || 
            pText.startsWith('therefore') ||
            pText.startsWith('provided') ||
            pText.startsWith('however')) {
          
          // Apply indentation class based on the last numbered level
...
function findParagraphByContext(doc, context) {
  try {
    // First try to find by data attributes if they were already applied
    const byAttributes = doc.querySelector(`p[data-num-id="${context.numberingId}"][data-num-level="${context.numberingLevel}"]`);
    if (byAttributes && !byAttributes.classList.contains('processed-list-item')) {
      return byAttributes;
    }
 ...
 * Create nested list
 * @param {Object} context - Paragraph context
 * @param {HTMLElement} parentItem - Parent list item
...
function createListItem(doc, paragraph, context) {
  try {
    ...
    // Apply prefix for display
    if (context.resolvedNumbering.formattedNumber) {
      listItem.setAttribute('data-prefix', context.resolvedNumbering.formattedNumber);
    }

...
// Helper function to find the matching heading ID in the document body
function findMatchingHeadingId(doc, tocText, numberingCtx) {
  ...
  // Try to match with headings first
  for (const heading of headings) {
    ...
    if (headingText === cleanTocText) {
      return heading.id || generateIdFromText(headingText);
    }
    
    // Try partial match (TOC text contains heading text or vice versa)
    if (cleanTocText.includes(headingText) || headingText.includes(cleanTocText)) {
      return heading.id || generateIdFromText(headingText);
    }
    
    // Try matching without punctuation and extra spaces
    const cleanHeadingText = headingText.replace(/[^\w\s]/g, '').replace(/\s+/g, ' ');
    const cleanTocTextNoPunct = cleanTocText.replace(/[^\w\s]/g, '').replace(/\s+/g, ' ');
  ...
  for (const anchor of anchors) {
    ...
    if (parentText.includes(cleanTocText) || cleanTocText.includes(parentText)) {
      return anchor.id;
    }
  }
...
// Helper function to generate a clean ID from text
...
function processTOCEntry(entry, index, tocContainer, styleInfo, numberingCtx) {
  try {
    ...
    if (
      ...
      // REMOVED: Manual numbering addition to prevent duplicate numbering
      // The CSS ::before pseudo-elements will handle numbering display
      // const textSpan = entry.querySelector(".docx-toc-text");
      // if (textSpan && numberingCtx.resolvedNumbering.fullNumbering) {
      //   const originalText = textSpan.textContent;
      //   textSpan.textContent = numberingCtx.resolvedNumbering.fullNumbering + " " + originalText;
      // }
    }
    ...
    if (textSpan && !textSpan.hasAttribute("role")) {
      ...
      // If this entry has a section ID from numbering, use that as the target
      let hrefTarget = "";
      
      // Find the matching heading in the document body to get the correct ID

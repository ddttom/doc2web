...
function processTOC(doc, styleInfo, numberingContext = null) {
  try {
    ...
    if (tocEntries.length > 0) {
      ...
      // This fixes entries that might not have been processed correctly (like the "Rationale" lines)
      const entriesInContainer =
        tocContainer.querySelectorAll(".docx-toc-entry");
      entriesInContainer.forEach((entry) => {
        ...
        if (
          ...
          // Entry doesn't have the right structure, try to fix it
          restructureTOCEntry(entry, styleInfo);
        }
      });
    }
  } catch (error) {
...
function processTOCEntry(entry, index, tocContainer, styleInfo, numberingCtx) {
  try {
    ...
    // Get text content and process structure
    const textContent = entry.textContent.trim();

    // Check if this entry already has the proper structure
    if (!entry.querySelector(".docx-toc-text")) {
      structureTOCEntry(entry, textContent, level);
    }

    // Apply numbering context if available
    if (
      numberingCtx &&
    ...
    // Move entry to TOC container if needed
    ...
    // Make the text part clickable if not already
    const textSpan = entry.querySelector(".docx-toc-text");
    if (textSpan && !textSpan.querySelector("a")) {
      // Create a link to the corresponding section
      ...
      const hrefTarget =
        textSpan.textContent
          ...
          .toLowerCase()
          .replace(/[^\w]+/g, "-")
          .replace(/^-|-$/g, "") || `toc-entry-${index}`;

      anchor.href = `#${hrefTarget}`;

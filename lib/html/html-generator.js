...
async function convertToStyledHtml(docxPath, styleInfo) {
  ...
  console.log(`Mammoth conversion result HTML length: ${result.value.length} characters`);
  
  // If the HTML is empty or very short, log a warning
  if (result.value.length < 100) {
    console.warn('WARNING: Mammoth conversion result HTML is very short or empty!');
    console.log('HTML content:', result.value);
  }
...
function applyStylesToHtml(html, css, styleInfo, cssFilename, metadata, trackChanges, options = {}) {
  try {
    ...
    // Log the HTML content length for debugging
    console.log(`HTML content length before JSDOM: ${html.length} characters`);
    ...
    if (document.body.children.length === 0) {
      console.warn('WARNING: Body is still empty after direct insertion!');
    }
    ...
    // Check if body is empty before serialization
    if (document.body.children.length === 0) {
      console.warn('WARNING: Body is empty before serialization!');
      
      // Try to add the original HTML content directly to the body
      document.body.innerHTML = html;
      console.log(`Added original HTML content directly to body before serialization`);
    }
    
    // Serialize back to HTML string
    const serialized = dom.serialize();
    
    // Log serialized HTML length for debugging
    console.log(`Serialized HTML length: ${serialized.length} characters`);

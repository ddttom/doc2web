...
function formatHtml(html) {
  try {
    // Minimal processing to avoid breaking structure, focus on readability
    // Normalize self-closing tags for consistency
    html = html.replace(/<([^>]+?)([^\s/])\/>/g, "<$1$2 />"); // Ensure space before self-closing slash if not present
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (openingTagMatch) {
        ...
        const isSelfClosingInTag =
          attributes.endsWith("/") || selfClosingElements.has(tagName);
        ...
        if (
          ...
          // e.g. <p>text</p>
          const closingTagOnSameLine = new RegExp(
            `</${tagName}\\s*>`,
            "i"
          ).test(line);
          if (!closingTagOnSameLine) {
            indentLevel++;
          }
        }
      }

...
function parseStyleNodeEnhanced(node) {
  try {
    ...
    const paragraphProps = pPrNode ? parseEnhancedParagraphProperties(pPrNode, styleId) : {};
 ...
 */
function parseEnhancedParagraphProperties(pPrNode, styleId = '') {
  const props = {};
  ...
  try {
    ...
    props.tabs = extractTabStops(pPrNode, styleId);
 ...
 * @param {Node} pPrNode - Paragraph properties node
 * @param {string} styleId - Style ID to check if it's a TOC style
 * @returns {Array} - Array of tab stop definitions
 */
function extractTabStops(pPrNode, styleId = '') {
  const tabs = [];
  ...
  try {
    ...
    if (tabsNode) {
      ...
      // Check if this is a TOC style
      const isTOCStyle = styleId && (
        styleId.toLowerCase().includes('toc') ||
        styleId.toLowerCase().includes('tableofcontents') ||
        styleId.toLowerCase().includes('contentsheading')
      );
      ...
      tabNodes.forEach(tabNode => {
        ...
        if (pos && val) {
          tabs.push({
            ...
            type: val,
            // For TOC styles, remove leader information to prevent page number generation
            leader: isTOCStyle ? 'none' : (leader || 'none'),
            leaderChar: isTOCStyle ? null : getLeaderChar(leader)
          });
        }
      });
    }

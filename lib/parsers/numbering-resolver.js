...
class NumberingSequenceTracker {
  ...
  // Enhanced generateSectionId method in NumberingSequenceTracker class
  generateSectionId(fullNumberingString, currentLevelIndex) {
    // Get the format type from the level definition
    const abstractNum = this.numberingDefs.abstractNums[this.abstractNumId];
    const levelDef = abstractNum?.levels?.[currentLevelIndex];
    const format = levelDef?.format || "decimal";
    
    // Create a clean ID that preserves the numbering format
    let cleanId = "section-";
    
    // Special handling for Roman numerals to ensure they're recognizable
    if (format === "upperRoman" || format === "lowerRoman") {
      // For Roman numerals, just use the raw number without additional processing
      // This helps with sections like "section-v" and "section-vi"
      const parts = fullNumberingString.split(".");
      cleanId += parts[parts.length - 1].toLowerCase();
    } else {
      // For other formats, use the standard cleaning approach
      cleanId += fullNumberingString
  ...
  // Enhanced method for display formatting with improved spacing
  buildDisplayNumbering(currentLevelIndex, currentLevelDef, abstractNum) {
    ...
    // with improved spacing between numbering segments
    ...
    currentLevelDef.parsedFormat.segments.forEach((segment) => {
      if (segment.type === "literal") {
        ...
        // Add spacing between level numbers if needed
        if (result.length > 0 && !result.endsWith(" ") && !result.endsWith(".") && !result.endsWith(")") && !result.endsWith("(")) {
          result += " "; // Add space between segments if needed
        }
        ...
        if (countForLevel !== undefined && defForLevel) {
          // Special handling for Roman numerals to ensure proper spacing
          const formattedNumber = this.formatSingleNumber(countForLevel, defForLevel.format);
          
          // Add the formatted number
          result += formattedNumber;
          
          // For Roman numerals, ensure there's a space after if another segment follows
          if ((defForLevel.format === "upperRoman" || defForLevel.format === "lowerRoman") && 

...
class NumberingSequenceTracker {
  ...
  formatNumbering(
    ...
    // Generate section ID from the full numbering
    const sectionId = this.generateSectionId(fullNumberingString, currentLevelIndex);
    ...
    return {
      ...
      fullNumbering: fullNumberingString,
      sectionId: sectionId, // NEW: Add section ID to the returned object
  ...
  // NEW: Add section ID generation method
  generateSectionId(fullNumberingString, currentLevelIndex) {
    ...
    return 'section-' + fullNumberingString
  ...
  buildHierarchicalNumbering(currentLevelIndex, currentLevelDef, abstractNum) {
    ...
    if (
      ...
      // Fallback if parsedFormat is missing or malformed (e.g. no lvlText for the level)
      // This ensures we try to produce *something* for the current level.
    ...
    currentLevelDef.parsedFormat.segments.forEach((segment) => {
      if (segment.type === "literal") {
        ...
        if (countForLevel !== undefined && defForLevel) {
          ...
          // console.warn(`In buildHierarchicalNumbering: Missing count or def for referenced level ${referencedLevelZeroBased} from placeholder %${referencedLevelOneBased}. AbstractNum: ${this.abstractNumId}, Current Level: ${currentLevelIndex}`);
          // result += `%${referencedLevelOneBased}`; // Keep placeholder if problematic, or omit
        }
      }
    });
    return result;

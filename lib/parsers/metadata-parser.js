...
function applyMetadataToHtml(document, metadata) {
  try {
    ...
    addMetaTag(head, document, 'author', metadata.core.creator);
...
function addDublinCoreMetadata(head, doc, coreMetadata) {
  ...
  if (coreMetadata.creator) {
    addMetaTag(head, doc, 'DC.creator', coreMetadata.creator);
  }
...
function addOpenGraphMetadata(head, doc, coreMetadata) {
  ...
  if (coreMetadata.creator) {
    addMetaTag(head, doc, 'article:author', coreMetadata.creator);
  }
...
function addTwitterCardMetadata(head, doc, coreMetadata) {
  ...
  if (coreMetadata.creator) {
    addMetaTag(head, doc, 'twitter:creator', coreMetadata.creator);
  }
}

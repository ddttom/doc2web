// lib/css/css-generator.js - Enhanced CSS generation with DOCX numbering support
...
`;
    ...
    // Add paragraph styles with numbering support
...
`;
    ...
    // ENHANCED: Generate CSS for DOCX-derived numbering definitions
...
.${className}[data-num-id="${numId}"][data-num-level="${numLevel}"]::before {
  ...
  ${levelDef.indentation?.hanging ? `margin-left: -${levelDef.indentation.hanging}pt;` : ''}
  display: inline-block;
  width: ${levelDef.indentation?.hanging || 24}pt;
...
/**
 * Generate CSS for DOCX-derived numbering definitions
...
`;
    ...
    // Generate styles for each level
...
/* Numbering format-specific styles */
ol.docx-numbered-list > li::before {
...
ul.docx-bulleted-list > li::before {
  content: "•";
  font-weight: bold;
}
...
/* Styles for lists created from DOCX numbering context */
li[data-numbering-id] {
  position: relative;
}

li[data-numbering-id][data-format="decimal"]::before {
  content: attr(data-number) ".";
}

li[data-numbering-id][data-format="lowerLetter"]::before {
  content: attr(data-number) ".";
}

li[data-numbering-id][data-format="upperLetter"]::before {
  content: attr(data-number) ".";
}

li[data-numbering-id][data-format="lowerRoman"]::before {
  content: attr(data-number) ".";
}

li[data-numbering-id][data-format="upperRoman"]::before {
  content: attr(data-number) ".";
}

...
function generateDOCXNumberingStyles(numberingDefs, styleInfo) {
  ...
  Object.values(numberingDefs.abstractNums || {}).forEach((abstractNum) => {
    ...
    if (allLevelCountersForAbstractNum) {
      css += `
body { /* Apply broad reset; specific list containers are better if identifiable */
    counter-reset: ${allLevelCountersForAbstractNum};
}
/* More targeted reset for list containers if they get data-abstract-num */
ol[data-abstract-num="${abstractNum.id}"], ul[data-abstract-num="${abstractNum.id}"], div[data-abstract-num="${abstractNum.id}"] {
    counter-reset: ${allLevelCountersForAbstractNum};
}
`;
    }
    ...
    Object.entries(abstractNum.levels || {}).forEach(
      ([levelIndexStr, levelDef]) => {
        ...
        let pIndentLeft = levelDef.indentation?.left || 0;
        let pTextIndent = 0;
        ...
        if (levelDef.indentation) {
          if (levelDef.indentation.hanging) {
            ...
            pTextIndent = -numRegionWidth; // First line of text is pulled left into the numRegion space.
          } else if (levelDef.indentation.firstLine) {
            pTextIndent = levelDef.indentation.firstLine;
            // If firstLine is positive, text starts indented. Number is usually before this.
            // numRegionWidth is implicitly the space before text-indent starts.
          }
        }
...
${itemSelector}::before {
  ...
  font-family: ${
    levelDef.runProps?.font?.ascii ? `"${levelDef.runProps.font.ascii}", ` : ""
  } ${
...
`;
        ...
        if (deeperLevelsToReset) {
          css += `
${itemSelector} { counter-reset: ${deeperLevelsToReset}; }
`;
        }
      }
    );
...
function generateTOCStyles(styleInfo) {
  /* Use full styleInfo for theme access */
...
.docx-toc { margin: 1em 0 2em 0; width: 100%; padding: 0; column-count: 1; -webkit-column-count: 1; -moz-column-count: 1; }
.docx-toc-heading {
  font-family: "${
  ...
  font-size: ${
    tocStyles.tocHeadingStyle?.fontSize || "16pt"
  }; /* Increased size */
  font-weight: bold; margin-bottom: 1em; /* Increased bottom margin */
  text-align: ${
    tocStyles.tocHeadingStyle?.alignment || "left"
  }; /* Default to left */
...
.docx-toc-entry { display: flex; align-items: baseline; width: 100%; margin-bottom: 0.3em; line-height: 1.4; overflow: visible; /* Allow overflow for debug, then clip */ }
.docx-toc-text { order: 1; flex-shrink: 0; /* Don't shrink text part */ padding-right: 0.5em; }
.docx-toc-text a {
  text-decoration: none; /* Remove underline from links in TOC text */
  color: inherit;       /* Ensure link color matches surrounding TOC text color */
}
.docx-toc-dots { order: 2; flex-grow: 1; flex-shrink: 1; position: relative; height: 1em; min-width: 1em; }
.docx-toc-dots::after {
  content: ""; position: absolute; bottom: 0.3em; left: 0; right: 0; height: 1px;
  background-image: ${
    leaderStyle.character === "."
      ? "radial-gradient(circle, currentColor 0.8px, transparent 1.2px)" /* Finer dots */
      : leaderStyle.character === "-"
      ? "linear-gradient(to right, currentColor 100%, transparent 0%)"
      : "linear-gradient(to right, currentColor 100%, transparent 0%)"
  }; /* Underscore as solid line */
  background-position: bottom;
  background-size: ${
    leaderStyle.character === "." ? "2.5px 1px" : "100% 1px"
  }; /* Adjust dot spacing or line */
  background-repeat: ${
    leaderStyle.character === "." ? "repeat-x" : "no-repeat"
  };
}
.docx-toc-pagenum { order: 3; text-align: right; padding-left: 0.5em; white-space: nowrap; font-variant-numeric: tabular-nums;}
`;
...
.docx-toc-level-${level} {
  ...
  font-size: ${
    style.fontSize || "10pt"
  }; /* Consistent base size for TOC entries */
  font-weight: ${
    level === 1
      ? style.bold === false
        ? "normal"
  ...
  padding-left: ${
    leftIndentPt > 0 ? leftIndentPt : baseIndent
  }pt; /* Prefer points, fallback to em */
  /* Ensure it's treated as a block to stack correctly */
  display: block; width: 100%; 
}
/* Add specific data attributes if TOC entries are numbered by the main numbering system */
.docx-toc-level-${level}[data-abstract-num] {
    padding-left: 0; /* Reset padding if DOCX numbering handles it */
    margin-left: ${
      leftIndentPt > 0 ? leftIndentPt : baseIndent
    }pt; /* Use margin for DOCX numbering */
}
`;
  });
  return css;
